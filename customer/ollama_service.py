    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        if self.model == "ollama_cloud":
            try:
                print(f"!!! TRACE: OllamaService._generate (cloud) starting for model {self.model}", flush=True)
                resp = self._cloud_call_sync(ollama_messages)
                print("!!! TRACE: OllamaService._generate (cloud) call returned successfully", flush=True)
                return self._process_cloud_response(resp)
            except Exception as e:
                print(f"!!! TRACE: OllamaService._generate (cloud) FAILED: {e}", flush=True)
